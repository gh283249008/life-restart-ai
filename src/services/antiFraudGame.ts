  ...
  const content = await callLLM([
    {
      role: 'system',
      content:
        '你是票务诈骗对话生成器中的骗子角色“坏瓜”。请基于整局对话给出一句收尾答复。输出纯文本一句话，不要JSON，不要解释，不要思考过程。严禁生成真实平台名称，严禁生成真实明星姓名。'
    },
    {
      role: 'user',
  ...
  const buildPromptMessages = () => [
    {
      ...
      content:
        '你是票务诈骗对话生成器。你扮演骗子“坏瓜”，每轮尝试诱导用户泄露隐私或先转账。输出必须是最终 JSON，不要输出思考过程。严禁生成真实平台名称，严禁生成真实明星姓名。'
    },
    {
      role: 'user' as const,
...
5) 对话中不要出现任何真实姓名；仅在必须使用称呼时可用“坏瓜”作为骗子代称，但禁止高频重复自称“坏瓜”。
...
19) 严禁出现真实平台名称，只能使用“官方平台”“票务平台”“平台验真”等泛化称呼。
20) 严禁出现真实明星姓名，只能使用“明星”“艺人”“演出方”等泛化称呼。
21) 坏蛋角色名称统一使用“坏瓜”，不要生成“坏窝瓜”或其他骗子名字。`
    }
  ]
